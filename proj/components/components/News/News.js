...
import { fetchNewssList, selectIsNew } from '../../../slices/news_list.js';
import { fetchNews , selectIsNews} from '../../../slices/news.js';
...
export default function Forum() {
  ...
  async function getPosts() {
    ...
    const data =  await dispath(fetchNews());
    handleSearch()
  }
...
useEffect(() => {
  ...
  const serth = useSelector(selectIsNews)
  ...
  async  function obnov () {
   ...
   function setting() {
    ...
    async function list_post(post) {
     await dispath(fetchNewssList(post._id));
     console.log(post._id)
    navigatioin.navigate("Страница поста новости")
  }
  ...
  const mappedList = PostStatus ? (
    ...
    filteredPosts.map((post, index) => (
      ...
      {post.image && <Image style={styles.image} source={{uri: `http://192.168.0.100:3030${post.image}`}} />}
